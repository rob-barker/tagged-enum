...
def test_make_constructs_from_raw_value():
    ...
    assert instance.payload == "built via make"
...
def test_payload_type_returns_declared_payload_type():
    assert _Tester.payload_type(_Tester.STRING.value) is str
    assert _Tester.payload_type(_Tester.NONE.value) is type(None)
